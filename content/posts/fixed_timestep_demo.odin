...
main :: proc() {
    ...
    for !rl.WindowShouldClose() {
        rl.BeginDrawing()
        defer rl.EndDrawing()
        rl.ClearBackground({50, 100, 255, 255})
        ...
        for flags, action in frame_input.actions {
            prev_down := .Down in tick_input.actions[action]
            tick_input.actions[action] = {}

            if (.Down in flags) != prev_down {
                if .Down in flags {
                    tick_input.actions[action] += {.Pressed}
                } else {
                    tick_input.actions[action] += {.Released}
                }
            }
...
game_view_offset :: proc(game: Game) -> rl.Vector2 {
    return {game.camera_pos.x - WINDOW_SIZE_X / 2, game.camera_pos.y - WINDOW_SIZE_X / 2}
}
...
game_tick :: proc(game: ^Game, input: Input, delta: f32) {
    ...
    if game.player_dash_timer > 0.0 {
        ...
        // game.player_vel = lerp_exp(game.player_vel, 0, delta * PLAYER_FRICT)
    }
...
game_draw :: proc(game: Game) {
    ...
    rl.DrawRectangle(0, 0, BOUNDS_X, BOUNDS_Y, rl.GRAY)

    rl.DrawRectangleV(game.player_pos - PLAYER_RAD, PLAYER_RAD * 2, rl.WHITE)
    ...
    for bullet in game.bullets {
        ...
        rl.DrawRectanglePro(
            {bullet.pos.x - BULLET_RAD, bullet.pos.y - BULLET_RAD, BULLET_RAD * 2, BULLET_RAD * 2},
            BULLET_RAD,
            bullet.timer * 500,
            {200, u8(150 + int(math.sin(bullet.timer) * 20)), 20, 255},
        )
    }
}
...
game_interpolate :: proc(result: ^Game, game, prev_game: Game, alpha: f32) {
    ...
    for &bullet, i in result.bullets {
        ...
        bullet.timer = lerp(prev_game.bullets[i].timer, game.bullets[i].timer, alpha)
    }
}
